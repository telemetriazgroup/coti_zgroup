...
async function seed() {
  ...
  try {
    ...
    // ── 3b. CATÁLOGO HTML v12 (EST/FRI/ACC/PTA; mismo dataset que v10-html) ───
    const CATALOG_V10 = require('./catalog-v10-html');
    const { importZgroupHtmlCatalogRows } = require('../lib/catalogV12Import');
    const v12Stats = await importZgroupHtmlCatalogRows(client, CATALOG_V10, { createdBy: adminId });
    if (v12Stats.inserted > 0 || v12Stats.categoriesCreated > 0) {
      console.log(
